...
params["handled_dir"] = "c:/guider/auto2/" #Directory to store handled files and logs
...
def auto(params): #Run this method to begin the guider
    ...
    if _debug: print "Connecting to telnet"
	
    #2.1 Telnet for sending commands to guider
    _use_telnet = params["use_telnet"]
    if _use_telnet: _t = telnetlib.Telnet(params["telnet_ip"],params["telnet_port"])

    if _debug: print "Creating global variables" 
    ...
    #2.3 Misc Variables
    _months = {1:"jan",2:"feb",3:"mar",4:"apr",5:"may",6:"jun",7:"jul",
    ...
    #3. METHODS
    if _debug: print "Method defintiions"
    ...
    def getNew(handleddir,remotedir,substring):
        ...
        try: remote_files = os.listdir(remotedir)
        except:
                        print remotedir
			output("Error reading file list.",_log)
			return []
        for x in handled_files: have[x] = True
        for x in remote_files:
			ignore = True if have.has_key(x) else False
        if not ignore and substring in x: new_files.append(x)
    ...
    def getStatus(path):
        ...
        try:
			#Extract Header Info
            obRA = fitsImage[0].header['OBRA']
            obDec = fitsImage[0].header['OBDEC']
            RAoff = fitsImage[0].header['RAOFF']
            Decoff = fitsImage[0].header['DECOFF']
	...
	now = localtime()
        ...
        remote_dir = os.path.join("S:/", str(year)+monthName+day_s) #Directory containing today's files on server
